...
| 接 Codex 或其他 Agent | [AI 协作入门](docs/training/ai-intent-to-flow.md) | `go run . -action srv` |
...
| 文件与表格 I/O | `screenshot`、`save_html`、`read_json`、`read_csv`、`read_excel`、`write_json`、`write_csv`、`write_excel`、`zip_compress`、`zip_extract` | 是 | 是 | 是 | 应保持同步，MCP 下受 `allow_file_access` 约束 |
...
如果你想系统查这一层“能力动作”，直接看 [docs/capability-actions/README.md](docs/capability-actions/README.md)。  
如果你想查的是命令行 `-action`，再看 [docs/actions/README.md](docs/actions/README.md)。

## 快速开始

如果你只是想先完成首次运行，请优先看上面的“5 分钟跑起来”。
这一节保留的是完整命令表和详细说明。
...
适合把运行产物打包交付、解开下载到本地的交接包，或把一个文件夹作为整体产物传递。
...
- `zip_compress` 支持 `source_path`、`file`、`folder`、`files`、`folders`、`paths`、`sources`
- `with.base_dir` 可控制压缩包内保存的相对路径
- `zip_extract` 会拒绝解出到 `save_path` 外的危险路径
- 密码能力使用传统 ZipCrypto 兼容格式
- 受限 Flow / MCP 上下文中需要 `allow_file_access=true`
...
| Skills 介绍 | 解释 `skill` 在 Codex / Agent 协作里解决什么问题，以及当前仓库附带的 skill | [docs/skills/README.md](docs/skills/README.md) |
| 金字塔课程总图 | 用四层结构解释教程为什么要先跑通、再结构化、再交付、再标准化 | [docs/tutorials/curriculum-overview.zh-CN.md](docs/tutorials/curriculum-overview.zh-CN.md) |
| 培训体系总览 | 面向实施、测试、开发和讲师的统一入口 | [docs/training/README.md](docs/training/README.md) |
| AI 协作入门 | 面向 Agent 的“用户意图 -> MCP -> Flow -> 执行修复”实战教程 | [docs/training/ai-intent-to-flow.md](docs/training/ai-intent-to-flow.md) |
...
3. 想系统理解教程结构时先看 [docs/tutorials/curriculum-overview.zh-CN.md](docs/tutorials/curriculum-overview.zh-CN.md)
4. 想学 Flow 交付时看 [docs/training/learning-path.md](docs/training/learning-path.md)
5. 想接 Agent / MCP 时重点看 [docs/training/ai-intent-to-flow.md](docs/training/ai-intent-to-flow.md)
