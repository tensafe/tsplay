...
- `ocr_request` 返回 `text/result/confidence/request_id/processing_time_ms`，业务 Flow 可以直接读取 `{{ocr_result.text}}`。
- 安全边界沿用 tsplay 现有策略：需要 `allow_http=true`，读图片和保存响应时需要 `allow_file_access=true`。

## 待办

- 补真实站点登录验证码的端到端示例：截图或下载验证码图片，再用 `ocr_request` 识别并填回表单。
- 增加 goddddocr 服务健康检查动作或模板：在业务 Flow 开始前确认 `/ready` 可用。
- 建立 Python ddddocr 与 Go goddddocr 的 golden fixtures 对比集，记录准确率和耗时差异。
