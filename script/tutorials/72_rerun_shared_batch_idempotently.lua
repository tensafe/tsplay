...
for _, row in ipairs(rows) do
    table.insert(detail_rows, {
        ...
        phone = row.phone,
        status = row.status,
        operator_name = row.operator or latest_operator
    })
end

before_count_row = db_query_one({
...
write_json("artifacts/tutorials/72-rerun-shared-batch-idempotently-lua.json", {
    lesson = "72",
    mode = "lua",
    latest_batch_id = latest_batch_id,
    payload_key = payload_key,
    input_file = input_file,
    before_count_row = before_count_row,
    transaction_result = transaction_result,
    after_count_row = after_count_row,
    summary_row = summary_row
})

print("reran shared batch idempotently:", tostring(latest_batch_id))
print("wrote artifacts/tutorials/72-rerun-shared-batch-idempotently-lua.json")
