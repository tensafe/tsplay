...
var GlobalPlayWrightFunc = []LuaFunction{
	...
	{"write_excel", write_excel, "将行数据写入本地 Excel 文件", "Write rows to a local Excel .xlsx file. Example: write_excel('reports/import-results.xlsx', results, {'source_row', 'status'}, 'Results'). Parameters: file_path (string) - The output .xlsx path; value (list) - Rows as objects, lists, or scalars; headers (list<string>, optional) - Explicit column order; sheet (string, optional) - Sheet name, defaults to Sheet1. Returns: file metadata."},
