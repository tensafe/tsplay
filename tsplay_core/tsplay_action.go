...
var GlobalPlayWrightFunc = []LuaFunction{
	...
	{"click_box", click_box, "点击元素内的检测框中心", "Click the center of a detection box relative to an element. Example: click_box('#captcha', {x1=20, y1=30, x2=80, y2=90}). Flow can pass image_path for automatic screenshot-to-element scaling. Parameters: selector (string) - Element used as the coordinate origin; box (table/list) - Detection box; index (number, optional) - Box index when passing a list of boxes."},
	...
	{"read_json", read_json, "读取本地 JSON 文件", "Read any local JSON file into a Lua/Flow value. Example: read_json('artifacts/import-results.json'). Parameters: file_path (string) - The JSON file path. Returns: object, list, primitive, or null depending on the JSON content."},
	{"read_csv", read_csv, "读取本地 CSV 为行列表", "Read a local CSV file into a list of row objects. Example: read_csv('imports/users.csv', 2, 100, 'source_row'). Parameters: file_path (string) - The CSV file path; start_row (int, optional) - Resume from a source row number; limit (int, optional) - Maximum number of rows; row_number_field (string, optional) - Add the source row number into each row object. Returns: list<object> using the first non-empty row as headers."},
	{"read_excel", read_excel, "读取本地 Excel 为行列表", "Read a local Excel .xlsx file into a list of row objects. Example: read_excel('imports/users.xlsx', 'Users', 'A2:B20', {'name', 'phone'}, 10, 50, 'source_row'). Parameters: file_path (string) - The .xlsx file path; sheet (string, optional) - Sheet name; range (string, optional) - Excel cell range such as A2:B20; headers (list<string>, optional) - Explicit headers when the range contains data rows only; start_row (int, optional) - Resume from a source row number; limit (int, optional) - Maximum number of rows; row_number_field (string, optional) - Add the source row number into each row object. Returns: list<object>."},
	{"write_json", write_json, "将值写入本地 JSON 文件", "Write any value to a local JSON file. Example: write_json('reports/import-results.json', results). Parameters: file_path (string) - The output path; value (any) - The value to encode as JSON. Returns: file metadata."},
	{"write_csv", write_csv, "将行数据写入本地 CSV 文件", "Write rows to a local CSV file. Example: write_csv('reports/import-results.csv', results, {'source_row', 'status'}). Parameters: file_path (string) - The output path; value (list) - Rows as objects, lists, or scalars; headers (list<string>, optional) - Explicit CSV column order. Returns: file metadata."},
	{"write_excel", write_excel, "将行数据写入本地 Excel 文件", "Write rows to a local Excel .xlsx file. Example: write_excel('reports/import-results.xlsx', results, {'source_row', 'status'}, 'Results'). Multi-sheet example: write_excel('reports/workbook.xlsx', {sheets={{name='Summary', value=summary_rows}, {name='Errors', value=error_rows, headers={'source_row', 'error'}}}}). Parameters: file_path (string) - The output .xlsx path; value (list|object) - Rows as objects, lists, or scalars, or a workbook object with sheets; headers (list<string>, optional) - Explicit column order for single-sheet output; sheet (string, optional) - Sheet name for single-sheet output, defaults to Sheet1. Returns: file metadata."},
	{"zip_compress", zip_compress, "压缩文件、文件列表或文件夹为 ZIP", "Create a ZIP archive from one file, a list of files, or folders. Example: zip_compress({file_path='reports/export.zip', files={'reports/a.csv', 'reports/b.json'}, password='secret'}). Parameters: file_path (string) - Output .zip path; source_path/file/files/folder/folders (string|string[]) - Inputs to archive; password (string, optional) - ZipCrypto-compatible password; base_dir (string, optional) - Base directory for archive paths. Returns: archive metadata."},
	{"zip_extract", zip_extract, "解压 ZIP 文件到目录", "Extract a ZIP archive into a directory. Example: zip_extract({file_path='reports/export.zip', save_path='reports/exported', password='secret'}). Parameters: file_path (string) - Input .zip path; save_path (string) - Output directory; password (string, optional) - Password for encrypted entries; overwrite (bool, optional) - Whether to replace existing files, defaults true. Returns: extraction metadata."},
	...
	{"send_email", send_email, "发送邮件通知", "Send an email through SMTP. Example: send_email({to={'ops@example.com'}, subject='TSPlay run finished', body='Import completed', attachments={'reports/run.xlsx'}, connection='alerts'}). Parameters: config (table) - Email settings such as to, cc, bcc, subject, body, html, attachments, headers, from_email, reply_to, connection, smtp, and timeout."},
	...
	{"db_insert", db_insert, "写入一行到数据库表", "Insert one row into a database table using database/sql. Example: db_insert({table='crawl_results', row={keyword='山东大学', title='示例'}, columns={'keyword', 'title'}, connection='reporting', driver='pgsql'}). Parameters: table (string) - Target table; row (object) - Column/value object; columns (list<string>, optional) - Explicit column order; connection (string, optional) - Named database connection; driver (string, optional) - mysql, pgsql, sqlserver, or oracle. Aliases such as postgres/postgresql are also accepted."},
	{"db_insert_many", db_insert_many, "批量写入多行到数据库表", "Insert multiple rows into a database table using database/sql. Example: db_insert_many({table='crawl_results', rows={{keyword='山东大学'}, {keyword='北京大学'}}, columns={'keyword'}, connection='reporting', driver='pgsql'}). Parameters: table (string) - Target table; rows (list<object>) - Rows to insert; columns (list<string>, optional) - Explicit column order; connection (string, optional) - Named database connection; driver (string, optional) - mysql, pgsql, sqlserver, or oracle."},
	{"db_upsert", db_upsert, "写入或更新一行到数据库表", "Insert or update one row in a database table using database/sql. Example: db_upsert({table='crawl_results', row={keyword='山东大学', rank=1}, key_columns={'keyword'}, connection='reporting', driver='pgsql'}). Parameters: table (string) - Target table; row (object) - Column/value object; key_columns (list<string>) - Conflict key columns; update_columns (list<string>, optional) - Columns to update when matched; connection (string, optional) - Named database connection; driver (string, optional) - mysql, pgsql, sqlserver, or oracle."},
