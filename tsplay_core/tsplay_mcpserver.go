...
const browserCDPBrowserStateGrantError = "browser_cdp_launch/browser_cdp_endpoint/browser_cdp_port/browser_cdp_executable/browser_cdp_user_data_dir requires allow_browser_state=true only for trusted browser sessions"
...
func withBrowserCDPPortInput(description string) mcp.ToolOption {
	return mcp.WithNumber("browser_cdp_port",
...
type browserCDPToolConfig struct {
	Launch      bool
	Endpoint    string
	Port        int
	Executable  string
...
func browserCDPConfigFromToolRequest(request mcp.CallToolRequest) browserCDPToolConfig {
	port, _, _ := browserCDPPortFromToolRequest(request)
	launch, _, _ := browserCDPLaunchFromToolRequest(request)
	endpoint, _, _ := browserCDPStringFromToolRequest(request, "browser_cdp_endpoint")
	executable, _, _ := browserCDPStringFromToolRequest(request, "browser_cdp_executable")
	userDataDir, _, _ := browserCDPStringFromToolRequest(request, "browser_cdp_user_data_dir")
	config := browserCDPToolConfig{
		Launch:      launch,
		Endpoint:    endpoint,
		Port:        port,
	...
	if config.Executable != "" || config.UserDataDir != "" {
		config.Launch = true
	}
	return config
}

func browserCDPLaunchFromToolRequest(request mcp.CallToolRequest) (bool, bool, error) {
	raw, ok := request.GetArguments()["browser_cdp_launch"]
	if !ok {
	...
	case string:
		...
		if trimmed == "" {
			return false, true, fmt.Errorf("browser_cdp_launch must be a boolean")
		}
...
func browserCDPStringFromToolRequest(request mcp.CallToolRequest, name string) (string, bool, error) {
	...
	trimmed := strings.TrimSpace(value)
	if trimmed == "" {
		return "", true, fmt.Errorf("%s cannot be blank", name)
	}
	return trimmed, true, nil
}

func (config browserCDPToolConfig) usesCDP() bool {
	return config.Launch || config.Endpoint != "" || config.Port != 0 || config.Executable != "" || config.UserDataDir != ""
}

func requestUsesBrowserCDP(request mcp.CallToolRequest) bool {
	return browserCDPConfigFromToolRequest(request).usesCDP()
}

func browserCDPPortFromToolRequest(request mcp.CallToolRequest) (int, bool, error) {
	raw, ok := request.GetArguments()["browser_cdp_port"]
	if !ok {
		return 0, false, nil
	}
	var port int
	...
	case int64:
		if value < 1 || value > 65535 {
			return 0, true, fmt.Errorf("browser_cdp_port must be between 1 and 65535")
		}
		port = int(value)
	case float64:
		if value < 1 || value > 65535 {
		...
		if value != float64(int(value)) {
			return 0, true, fmt.Errorf("browser_cdp_port must be an integer between 1 and 65535")
		}
		port = int(value)
	case json.Number:
...
func validateBrowserCDPToolRequest(request mcp.CallToolRequest) error {
	if _, _, err := browserCDPLaunchFromToolRequest(request); err != nil {
		return err
	}
	for _, name := range []string{"browser_cdp_endpoint", "browser_cdp_executable", "browser_cdp_user_data_dir"} {
		if _, _, err := browserCDPStringFromToolRequest(request, name); err != nil {
			return err
		}
	}
	...
	config := browserCDPConfigFromToolRequest(request)
	browserConfig := FlowBrowserConfig{
		CDPEndpoint:    config.Endpoint,
		CDPPort:        config.Port,
		CDPLaunch:      config.Launch,
		CDPExecutable:  config.Executable,
		CDPUserDataDir: config.UserDataDir,
	}
...
func applyBrowserCDPOverrideFromToolRequest(flow *Flow, request mcp.CallToolRequest) {
	config := browserCDPConfigFromToolRequest(request)
	if !config.usesCDP() {
		return
	}
	...
	flow.Browser.CDPLaunch = config.Launch
	flow.Browser.CDPEndpoint = config.Endpoint
	flow.Browser.CDPPort = config.Port
	flow.Browser.CDPExecutable = config.Executable
	flow.Browser.CDPUserDataDir = config.UserDataDir
}
...
func registerTSPlayFlowTools(mcpServer *server.MCPServer, options TSPlayMCPServerOptions) {
	...
	mcpServer.AddTool(mcp.NewTool("tsplay.draft_flow",
		...
		mcp.WithBoolean("browser_cdp_launch",
			mcp.Description("Launch a local Chrome/Chromium/Edge with remote debugging enabled, then attach over CDP. Auto-detects the executable when browser_cdp_executable is omitted. Requires allow_browser_state=true."),
		),
		mcp.WithString("browser_cdp_endpoint",
			mcp.Description("Optional Chromium CDP endpoint to attach to instead of launching a new browser, for example ws://127.0.0.1:9222/devtools/browser/..., http://127.0.0.1:9222, or 127.0.0.1:9222/json/version. Requires allow_browser_state=true."),
		),
		withBrowserCDPPortInput("Optional local Chromium remote-debugging-port to attach to instead of launching a new browser, for example 9222. Requires allow_browser_state=true."),
		mcp.WithString("browser_cdp_executable",
			mcp.Description("Optional Chrome/Chromium/Edge executable path for browser_cdp_launch. When omitted TSPlay searches common macOS, Windows, and Linux locations. Requires allow_browser_state=true."),
		),
		mcp.WithString("browser_cdp_user_data_dir",
			mcp.Description("Optional user data directory for browser_cdp_launch. Defaults under the artifact root. Requires allow_browser_state=true."),
		),
	...
	mcpServer.AddTool(mcp.NewTool("tsplay.observe_page",
		...
		mcp.WithBoolean("allow_browser_state",
			mcp.Description("Allow observe_page to attach to a real browser over CDP. Defaults to false."),
		),
		mcp.WithNumber("timeout",
	...
	mcpServer.AddTool(mcp.NewTool("tsplay.run_flow",
		...
		mcp.WithString("browser_cdp_endpoint",
			mcp.Description("Optional Chromium CDP endpoint override to attach to instead of launching a new browser, for example ws://127.0.0.1:9222/devtools/browser/..., http://127.0.0.1:9222, or 127.0.0.1:9222/json/version. Requires allow_browser_state=true."),
		),
		withBrowserCDPPortInput("Optional local Chromium remote-debugging-port override to attach to instead of launching a new browser, for example 9222. Requires allow_browser_state=true."),
		mcp.WithString("browser_cdp_executable",
			mcp.Description("Optional Chrome/Chromium/Edge executable path for browser_cdp_launch. When omitted TSPlay searches common macOS, Windows, and Linux locations. Requires allow_browser_state=true."),
		),
		mcp.WithString("browser_cdp_user_data_dir",
...
func draftFlowPayloadWithOptions(
	...
	if observation == nil {
		...
		if err := validateBrowserCDPToolRequest(request); err != nil {
			return map[string]any{
				"ok":       false,
				"error":    err.Error(),
		...
		if requestUsesBrowserCDP(request) && !security.AllowBrowserState {
			return map[string]any{
				"ok":       false,
				"error":    browserCDPBrowserStateGrantError,
				"security": securityResolution,
			}
		}
		cdpConfig := browserCDPConfigFromToolRequest(request)
		...
		observation, err = ObservePage(PageObservationOptions{
			...
			CDPLaunch:      cdpConfig.Launch,
			CDPEndpoint:    cdpConfig.Endpoint,
			CDPPort:        cdpConfig.Port,
			CDPExecutable:  cdpConfig.Executable,
			CDPUserDataDir: cdpConfig.UserDataDir,
			ArtifactRoot:   options.ArtifactRoot,
			Security:       &security,
...
func handleObservePageToolWithOptions(
	...
	usesCDP := requestUsesBrowserCDP(request)
	if usesCDP && !security.AllowBrowserState {
		return newTSPlayToolResult("tsplay.observe_page", map[string]any{
			"ok":       false,
			"error":    browserCDPBrowserStateGrantError,
			"security": securityResolution,
	...
	cdpConfig := browserCDPConfigFromToolRequest(request)
	var grants *FlowSecurityPolicy
	if usesCDP {
		grants = &security
	}
	runHandle, runCtx, err := beginTSPlayBrowserRun(ctx, request, "tsplay.observe_page", options, grants)
	if err != nil {
		return newTSPlayToolResult("tsplay.observe_page", map[string]any{
			"ok":    false,
			"error": err.Error(),
...
func handleRunFlowToolWithOptions(
	...
	applyBrowserCDPOverrideFromToolRequest(flow, request)
	if err := ValidateFlow(flow); err != nil {
		payload := map[string]any{
			"ok":       false,
			"error":    err.Error(),
...
func buildFlowActionManifest() []map[string]any {
	...
	descriptions["ocr_request"] = "Send an image file to a goddddocr-compatible OCR HTTP service and return text, confidence, optional probability details, and response metadata."
	...
	for _, name := range FlowActionNames() {
		...
		if name == "write_excel" {
			...
			item["notes"] = []string{
				"Value should be a list of row objects, row lists, or scalars.",
				"Value can also be a workbook object shaped like {sheets:[{name, value|rows, headers?}, ...]} for multi-sheet output.",
