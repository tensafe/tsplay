...
func BuildFlowJSONSchema() map[string]any {
	stepProperties := map[string]any{
		...
		"source_path":       map[string]any{"description": "Input path for zip_compress, or archive path alias for zip_extract. Can be a string or list when compressing."},
		"folder":            map[string]any{"type": "string", "description": "Folder to include in zip_compress."},
		"folder_path":       map[string]any{"type": "string", "description": "Folder to include in zip_compress."},
		"files":             map[string]any{"type": "array", "items": map[string]any{"type": "string"}},
		"folders":           map[string]any{"type": "array", "items": map[string]any{"type": "string"}},
		"save_path":         map[string]any{"type": "string"},
		"password":          map[string]any{"type": "string", "description": "Optional password for zip_compress or zip_extract."},
		"base_dir":          map[string]any{"type": "string", "description": "Optional base directory for zip_compress archive entry names."},
		...
		"properties": map[string]any{
			...
			"browser": map[string]any{
				...
				"properties": map[string]any{
					...
					"cdp_launch":         map[string]any{"type": "boolean", "description": "Launch a local Chrome/Chromium/Edge with remote debugging enabled, then attach over CDP. TSPlay auto-detects common browser locations when cdp_executable is omitted."},
					"cdp_endpoint":       map[string]any{"type": "string", "description": "Attach to an existing Chromium/Chrome browser over CDP. Accepts ws://..., http://127.0.0.1:9222, or pasted local forms like 127.0.0.1:9222/json/version."},
					"cdp_port":           map[string]any{"type": "integer", "minimum": 1, "maximum": 65535, "description": "Attach to an existing local Chromium/Chrome remote-debugging-port over CDP, or use this port when cdp_launch is true."},
					"cdp_executable":     map[string]any{"type": "string", "description": "Optional Chrome/Chromium/Edge executable path for cdp_launch."},
					"cdp_user_data_dir":  map[string]any{"type": "string", "description": "Optional user data directory for cdp_launch. Defaults under the artifact root."},
...
func flowSchemaGenerationRules() []string {
	return []string{
		...
		"Put session-wide browser concerns such as headless, use_session, storage_state, cdp_launch, cdp_endpoint/cdp_port, timeout, viewport, user_agent, or persistent profile naming in the top-level browser block.",
		"Use browser.cdp_launch/cdp_endpoint/cdp_port only when the user explicitly wants a trusted Chrome/Chromium/Edge session; do not combine CDP mode with use_session, storage_state, persistent profile, or user_agent.",
